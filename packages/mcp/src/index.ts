...
server.registerTool(
  ...
  async ({ projectId, code, title, phase, estimateMinutes }) =>
    json(await apiClient.createTask({ projectId, code, title, phase, estimateMinutes })),
);
...
server.registerTool(
  "report_work",
  {
    ...
    description:
      "Report time and AI usage on a task. Identify the task either by taskId, OR by projectName + taskCode (resolved server-side; set createMissing=true to create the project/task if they don't exist). The server computes the real AI cost (tokens x model price) and records a time entry. Report minutes of human time and, if a model did work, one aiRuns entry per model.",
    inputSchema: {
...
server.registerTool(
  "set_model_price",
  {
    ...
    description:
      "Add or update a model's price (USD per 1M tokens). If a price for provider+model already exists, it is updated; otherwise created. report_work uses this to compute AI cost.",
    inputSchema: {
  ...
  async ({ provider, model, inputPer1M, outputPer1M }) => {
    const existing = (await apiClient.listModelPrices()).find(
